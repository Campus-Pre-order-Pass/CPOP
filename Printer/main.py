...
class OrderInvoiceGenerator:
    ...
    def draw_init(self, shop):   
        ...
        self.image = Image.new('RGB', (self.width, self.image_height), self.background_color)
    ...
    # 畫店家標誌
    def draw_logo(self):   
        image_x = (self.width - self.logo_width)  - 25
        self.y_position = 0
    ...
    def draw_order_time(self):  
        ...
        self.draw.text((x, self.y_position), time_text, fill=(0, 0, 0), font=self.font)
    ...
    def draw_order_number(self):
        ...
        self.draw.text((x, self.y_position), number_text, fill=(0, 0, 0), font=self.font)
    ...
    def draw_table_header(self):
        ...
        for i, title in enumerate(title_row):
            ...
            line_text = title
            self.draw.text((cell_x-100, self.y_position - 50), line_text, fill=(0, 0, 0), font=self.font)
        ...
        for detail in self.order_details[3:99]:
            for i, cell in enumerate(detail):
                ...
                if line_text and i == 2:  # 內容
                    self.y_position += 42
                    font = ImageFont.truetype(self.font_path, self.font_size - 24)
                    self.draw.text((cell_x-100, self.y_position-20), line_text, fill=(0, 0, 0), font=font)
                    self.y_position -= 38
                else:  # 備註
                    self.y_position -= 4
                    font = ImageFont.truetype(self.font_path, self.font_size - 10)
                    self.draw.text((cell_x-90, self.y_position-20), line_text, fill=(0, 0, 0), font=font)
    ...
    def draw_total_amount(self):  
        ...
        x = (self.width - self.table_width)
        self.draw.text((x-40, self.y_position-50), total_text, fill=(0, 0, 0), font=self.font)
...
# invoice_generator.generate_invoice(shop="A", order_details=order_details_example, print_invoice=True, show_invoice=False)
# invoice_generator.generate_invoice(shop="B", order_details=order_details_example, print_invoice=True, show_invoice=False)
